...
void main() {
  ...
  group('bookingDateFor', () {
    test('before 8 PM → today', () {
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 14, 0)),
          DateTime(2026, 4, 30));
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 0, 0)),
          DateTime(2026, 4, 30));
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 18, 59)),
          DateTime(2026, 4, 30));
    });

    test('exactly 8 PM → tomorrow', () {
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 20, 0)),
          DateTime(2026, 5, 1));
    });

    test('after 8 PM → tomorrow', () {
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 22, 30)),
          DateTime(2026, 5, 1));
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 23, 59)),
          DateTime(2026, 5, 1));
    });

    test('month boundary — last day of April after 8 PM → May 1', () {
      expect(BookingWindow.bookingDateFor(DateTime(2026, 4, 30, 21, 0)),
          DateTime(2026, 5, 1));
    });
  });
